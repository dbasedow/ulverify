...
fn main() {
    ...
    let candidate_a = well_known_aasa_from_url(&url);
    println!(
        "trying to fetch Apple app site association file {}",
        candidate_a
    );

    let p = fetch_and_check(candidate_a, url.path()).and_then(|matches| {
        for m in matches {
            println!("{}", m);
        }
        Ok(())
...
fn aasa_match(app: &AppLinkDetail, path: &str) -> bool {
...
fn well_known_aasa_from_url(uri: &Uri) -> Uri {
...
fn root_aasa_from_url(uri: &Uri) -> Uri {
    let host = uri.host().unwrap();
    let mut parts = Parts::default();
    parts.authority = Some(host.parse().unwrap());
...
fn test_aasa_from_url() {
    assert_eq!(
        "https://example.com/.well-known/apple-app-site-association"
            .parse::<Uri>()
            .unwrap(),
        well_known_aasa_from_url(&"http://example.com/foo/bar?hello=world".parse::<Uri>().unwrap())
    );
    assert_eq!(
        "https://example.com/apple-app-site-association"
            .parse::<Uri>()
            .unwrap(),
        root_aasa_from_url(&"http://example.com/foo/bar?hello=world".parse::<Uri>().unwrap())
    );
}
...
fn aasa_match_path(pattern: &str, path: &str) -> bool {
